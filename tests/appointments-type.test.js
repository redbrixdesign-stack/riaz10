...
(async () => {
  ...
  const scannedFittingOnSalesDay = feature.renderAddForm({ date: '2026-08-20', scannedType: 'fitting' });
  assert.match(scannedFittingOnSalesDay, /value="fitting" selected/, 'a confidently scanned fitting is selected even on a configured sales day');
  const invalidScannedType = feature.renderAddForm({ date: '2026-08-20', scannedType: 'delivery' });
  assert.doesNotMatch(invalidScannedType, /value="delivery"/, 'an unrecognised scanned type is never added to the form');
